...
<div class="shell" [class.collapsed]="sidebarCollapsed()">
  ...
  styles: [`
    .shell {
      ...
      --sb: var(--mf-sidebar-w);
      transition: --sb .25s;
      &.collapsed { --sb: var(--mf-sidebar-mini); }
    }
    ...
    .sidebar {
      ...
      overflow: hidden;
      z-index: 100;
      .collapsed & { width: var(--mf-sidebar-mini); min-width: var(--mf-sidebar-mini); }
    }
    ...
    .logo {
      ...
      .logo-text {
        font-size: .95rem; color: white; white-space: nowrap;
        letter-spacing: -.01em;
        strong { color: #818cf8; }
      }
    ...
    .nav-list {
      flex: 1; padding: .5rem; display: flex; flex-direction: column; gap: 2px;
      overflow-y: auto; overflow-x: hidden;
    }

    .nav-item {
      display: flex; align-items: center; gap: .75rem;
      padding: .65rem .75rem; border-radius: var(--mf-radius-sm);
      color: var(--mf-gray-400); text-decoration: none;
      transition: background .15s, color .15s;
      cursor: pointer; white-space: nowrap; min-height: 44px;
      &:hover { background: rgba(255,255,255,.06); color: white; }
      &.active { background: var(--mf-blue); color: white; }
      .collapsed & { padding: .65rem; justify-content: center; }
    }

    .nav-icon { font-size: 20px; flex-shrink: 0; font-variation-settings: 'FILL' 1; }
    .nav-label { font-size: .875rem; font-weight: 500; .collapsed & { display: none; } }
    .nav-badge {
    ...
    .sidebar-footer {
      ...
      .collapsed & { padding: .75rem; justify-content: center; }
    }

    .user-info {
      display: flex; align-items: center; gap: .65rem; flex: 1; overflow: hidden;
      .collapsed & { display: none; }
    }

    .user-avatar {
      width: 32px; height: 32px; border-radius: 50%;
      background: var(--mf-blue); color: white;
    ...
    .topbar {
      height: var(--mf-topbar-h); background: var(--mf-surface);
      border-bottom: 1px solid var(--mf-border);
      display: flex; align-items: center; justify-content: space-between;
      padding: 0 1.5rem; flex-shrink: 0;
      box-shadow: var(--mf-shadow-sm); z-index: 50;
    }

    .topbar-left { display: flex; align-items: center; gap: .75rem; }
    .topbar-title { font-size: 1rem; font-weight: 600; color: var(--mf-gray-800); }
    ...
    .status-chip {
      ...
      &.open  { background: var(--mf-green-light); color: var(--mf-green); }
      &.closed { background: var(--mf-gray-100);  color: var(--mf-gray-500); }
    }

    .status-dot {
      width: 7px; height: 7px; border-radius: 50%;
      .open  & { background: var(--mf-green); animation: pulse 2s infinite; }
      .closed & { background: var(--mf-gray-400); }
    }
...
navItems = computed(() => {
  ...
  currentTitle = computed(() => {
    ...
    return this.titleMap[seg] ?? 'MercadoFácil';
  });
