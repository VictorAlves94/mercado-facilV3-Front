...
<div class="mf-page">
  <div class="mf-section-header">
    <div>
      <h1 class="mf-page-title">Usuários</h1>
      <p class="mf-page-subtitle">Gerencie os acessos ao sistema</p>
    </div>
      ...
      <tbody>
        @for (u of usuarios(); track u.id) {
          <tr [class.inativo]="!u.ativo">
            ...
            <td>{{ u.email }}</td>
            <td>
              <span class="mf-badge" [class]="u.perfil === 'ADMIN' ? 'danger' : u.perfil === 'GERENTE' ? 'warning' : 'info'">
                {{ u.perfil }}
              </span>
            </td>
            <td>{{ u.lojaNome || '—' }}</td>
            ...
            <td class="acoes-cell">
              @if (u.perfil !== 'ADMIN') {
                ...
                <button mat-icon-button [matTooltip]="u.ativo ? 'Desativar' : 'Ativar'"
  ...
  @if (modalAberto()) {
    ...
    <div class="usuario-modal">
      ...
      <form [formGroup]="form" (ngSubmit)="salvar()" class="modal-body">
        ...
        <mat-form-field appearance="outline">
          <mat-label>{{ editando() ? 'Nova senha (deixe em branco para manter)' : 'Senha' }}</mat-label>
  ...
  styles: [`
    .usuarios-table {
      ...
      th { padding: .75rem 1rem; text-align: left; font-size: .75rem; font-weight: 600;
           text-transform: uppercase; letter-spacing: .06em; color: var(--mf-gray-500);
           border-bottom: 1px solid var(--mf-border); background: var(--mf-gray-50); }
      td { padding: .75rem 1rem; font-size: .875rem; border-bottom: 1px solid var(--mf-border); }
    ...
    .user-avatar-sm {
      width: 28px; height: 28px; border-radius: 50%; background: var(--mf-blue);
    ...
    .usuario-modal {
      ...
      z-index: 201; box-shadow: 0 20px 60px rgba(0,0,0,.2);
    }
    .modal-header {
...
export class UsuariosComponent implements OnInit {
  ...
  editarUsuario(u: any) {
    this.editando.set(u);
    this.form.patchValue({ nome: u.nome, email: u.email, senha: '', perfil: u.perfil, lojaId: u.lojaId });
    this.form.get('senha')!.clearValidators();
    this.form.get('senha')!.updateValueAndValidity();
    this.modalAberto.set(true);
  }

  fecharForm() { this.modalAberto.set(false); }
  ...
  salvar() {
    ...
    obs.subscribe({
      next: () => {
        this.snack.open(this.editando() ? 'Usuário atualizado!' : 'Usuário criado!', '', { duration: 3000 });
        this.fecharForm();
        this.carregar();
        this.saving.set(false);
      },
      error: err => {
        this.snack.open(err.error?.message || 'Erro ao salvar', '', { duration: 4000 });
        this.saving.set(false);
      }
    });
  }
  ...
  alterarStatus(u: any) {
    ...
    this.svc.alterarStatus(u.id, !u.ativo).subscribe({
      next: () => { this.snack.open(`Usuário ${acao}do!`, '', { duration: 3000 }); this.carregar(); },
